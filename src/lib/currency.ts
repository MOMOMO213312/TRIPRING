...
/**
 * Currency name in the UI language: the DB's Arabic name for Arabic, and otherwise the browser's own
 * localised name (so Turkish gets "Mısır Lirası"), falling back to the DB's English name.
 */
export function currencyName(c: CurrencyInfo, lang: string): string {
  if (lang.startsWith("ar")) return c.name_ar;
  if (!lang.startsWith("en")) {
    try {
      const localized = new Intl.DisplayNames([lang], { type: "currency" }).of(c.code);
      if (localized && localized !== c.code) return localized;
    } catch {
      /* unsupported locale — use the English name */
    }
  }
  return c.name_en;
}
