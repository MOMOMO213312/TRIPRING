...
export const SCORE_TIER_COLORS: Record<ScoreTier, { fg: string; bg: string; ring: string }> = {
  excellent: { fg: "var(--color-success)", bg: "var(--color-score-bg)", ring: "var(--color-success-bg)" },
  good: { fg: "var(--color-primary)", bg: "var(--color-primary-light)", ring: "var(--color-primary-ring)" },
  fair: { fg: "var(--color-score-gray)", bg: "var(--color-surface-muted)", ring: "var(--color-border-subtle)" },
};
...
export function rankBadgeStyle(rank: number): { bg: string; text: string } {
  if (rank === 1) return { bg: "var(--color-rank-gold)", text: "white" };
