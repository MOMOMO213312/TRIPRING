 ...
 * row joined with its flight `deal` and `tripgo_deal` — never by
 * `additional_services`. Transport price is the agency's own selling price
...
export function TripGoCard({ bundle, catalog }: { bundle: TripGoBundleJoined; catalog: Catalog }) {
  ...
  const transferPrice = bundle.agency_selling_price ?? transport.price;
  const total = tripGoTotal(deal.price, transferPrice);
