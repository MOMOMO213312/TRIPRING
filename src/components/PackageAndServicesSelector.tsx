import { classifyService, includedServicesFor, packagePrice, PACKAGE_OPTIONS } from "../lib/packages";
...
export function selectorTotal(
  ...
  const pkg = PACKAGE_OPTIONS.find((p) => p.id === selectedPackage) ?? PACKAGE_OPTIONS[0];
...
export function PackageAndServicesSelector({
  ...
  const activePkg = PACKAGE_OPTIONS.find((p) => p.id === selectedPackage) ?? PACKAGE_OPTIONS[0];
  const includedIds = new Set(includedServicesFor(activePkg, services).map((s) => s.id));
  const total = selectorTotal(basePrice, selectedPackage, services, checkedServiceIds);
