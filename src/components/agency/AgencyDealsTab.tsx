  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {deals.length === 0 ? (
          <p className="py-8 text-center text-sm text-gray-500">لا توجد عروض بعد</p>
        ) : (
          deals.map((deal) => (
            <Card key={deal.id} className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-400">ينتهي: {new Date(deal.expires_at).toLocaleString("ar-EG")}</p>
              </div>
