...
import { usePackageOptions, type PackageOption } from "../../lib/packages";
import { whatsAppLink } from "../../lib/utils";
import type { BookingRow, BookingStatus } from "../../types/database";

function farePackageLabel(tier: string | null | undefined, packageOptions: PackageOption[]): string | null {
  if (!tier) return null;
  return packageOptions.find((p) => p.id === tier)?.label ?? tier;
}
          ...
          {bookings.map((b) => (
            <Card key={b.id} className="space-y-3">
              ...
              <div className="grid gap-1 text-sm text-slate-700 sm:grid-cols-2">
                ...
                {farePackageLabel(b.fare_package_tier, packageOptions) ? (
                  <p>
                    🎁 الباقة: {farePackageLabel(b.fare_package_tier, packageOptions)}
                    {b.fare_package_markup ? ` (+${b.fare_package_markup} ${b.currency ?? ""})` : ""}
                  </p>
                ) : null}
