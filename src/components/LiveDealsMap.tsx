      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500">
          <span className="inline-flex items-center gap-1">
            <span className="size-2 rounded-full bg-danger" /> عروض ساخنة
          </span>
          <span className="inline-flex items-center gap-1">
            <span className="size-2 rounded-full bg-success" /> عروض جيدة
          </span>
        </div>
      </div>

      <svg viewBox="0 0 1000 500" className="w-full rounded-lg bg-navy-deep">
        {CONTINENTS.map((d, i) => (
          <path key={i} d={d} fill="var(--color-navy-slate)" stroke="var(--color-navy-steel)" strokeWidth={1} />
        ))}
          ...
          points.map((p) => {
            ...
            return (
              <g key={p.code}>
                ...
                <circle cx={p.pos.x} cy={p.pos.y} r={4} fill={color} stroke="var(--color-navy-deep)" strokeWidth={1.5}>
                  <title>
