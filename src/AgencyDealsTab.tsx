...
import { AgencyAmadeusPasteModal } from "./AgencyAmadeusPasteModal";
...
export function AgencyDealsTab({ agencyId }: { agencyId: string }) {
  ...
  const [showAmadeusPaste, setShowAmadeusPaste] = useState(false);
    ...
    <div className="space-y-4">
      ...
      {!showForm && !editingDeal ? (
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => setShowAmadeusPaste(true)}>
            📋 لصق من أماديوس
          </Button>
        </div>
      ) : null}
      ...
      {showAmadeusPaste ? (
        <AgencyAmadeusPasteModal
          ...
          airports={airports}
          airlines={airlines}
          onClose={() => setShowAmadeusPaste(false)}
          onImported={reload}
        />
      ) : null}
