...
function StatCard({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <Card className={highlight ? "border-primary bg-primary-light text-center" : "text-center"}>
      <p className="text-xs text-slate-500">{label}</p>
      <p className={`font-latin mt-1 text-xl font-extrabold ${highlight ? "text-primary" : "text-slate-900"}`}>
        {value}
      </p>
    </Card>
  );
