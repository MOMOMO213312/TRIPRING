      ...
      <Card className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl bg-primary-light p-4 text-center">
            ...
            <p className="font-latin mt-1 text-2xl font-extrabold text-primary">
          ...
          <div className="rounded-xl bg-score-bg p-4 text-center">
            <p className="text-xs font-medium text-slate-500">أنت توفر</p>
            <p className="font-latin mt-1 text-2xl font-extrabold text-success">
              {savingsAmount ? `${formatPrice(savingsAmount, currency)}` : "—"}
            </p>
          </div>
        </div>
        {savings ? (
          <p className="rounded-lg bg-score-bg px-4 py-2.5 text-center text-sm font-semibold text-success">
      ...
      <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 sm:sticky sm:bottom-4 sm:shadow-lg">
        ...
        <div className="flex gap-2 text-sm">
          <button
            type="button"
            onClick={handleShare}
            className="flex-1 rounded-lg border border-slate-200 py-2 font-semibold text-slate-600 transition hover:border-primary hover:text-primary"
          >
...
function ReasonIcon({ type }: { type: "down" | "seats" | "nonstop" | "fresh" | "score" }) {
  const common = "size-4 shrink-0 text-success";
  if (type === "down") {
    return (
