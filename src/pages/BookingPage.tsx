        ...
        {STEPS.map((label, i) => (
          <div
            ...
            className={`flex-1 rounded-lg px-2 py-2 text-center text-xs font-semibold sm:text-sm ${
              i === step ? "bg-accent text-white" : i < step ? "bg-primary-ring text-navy-teal" : "bg-slate-100 text-slate-500"
            }`}
          >
            {label}
          </div>
            ...
            {PAYMENT_METHODS.map((pm) => (
              <label
                ...
                className={`block cursor-pointer rounded-xl border p-4 ${
                  paymentMethod === pm.value ? "border-accent bg-primary-light" : "border-slate-200"
                }`}
              >
                <input
                  type="radio"
